...
void Array<T>::insert(const int POS, const T VALUE) {
    ...
    T* tempArray = new T[static_cast<std::size_t>(_size + 1)];
...
void Array<T>::remove(const int POS) {
    ...
    // create new array of size - 1
    T* tempArray = (_size > 1) ? new T[static_cast<std::size_t>(_size - 1)] : nullptr;
...
template<typename T>
void Array<T>::merge(const int LEFT, const int MID, const int RIGHT) {
    const int LEFT_LEN = MID - LEFT + 1;
    const int RIGHT_LEN = RIGHT - MID;

    T* leftArr = new T[static_cast<std::size_t>(LEFT_LEN)];
    T* rightArr = new T[static_cast<std::size_t>(RIGHT_LEN)];

    for (int i = 0; i < LEFT_LEN; ++i) {
        leftArr[i] = _pArray[LEFT + i];
    }
    for (int j = 0; j < RIGHT_LEN; ++j) {
        rightArr[j] = _pArray[MID + 1 + j];
    }
...
template<typename T>
void Array<T>::mergeSort(const int LEFT, const int RIGHT) {
    if (LEFT >= RIGHT) {
        return;
    }
...
int Array<T>::search(const T VALUE) const {
    ...
    while (left <= right) {
        ...
        if (_pArray[MID] == VALUE) {
            int first = MID;
            while (first > 0 && _pArray[first - 1] == VALUE) {
                --first;
            }
            return first;
        }
