...
private: 
    ...
    Node* mergeSort(Node* const HEAD, const int LENGTH);
...
template<typename T>
typename LinkedList<T>::Node* LinkedList<T>::mergeLists(Node* left, Node* right) {
    if (left == nullptr) {
        return right;
    }
    if (right == nullptr) {
    ...
    while (left != nullptr && right != nullptr) {
        if (left->value <= right->value) {
            Node* pNext = left->pNext;
            appendNode(left);
            left = pNext;
        } else {
            Node* pNext = right->pNext;
            appendNode(right);
            right = pNext;
        }
    }
...
template<typename T>
typename LinkedList<T>::Node* LinkedList<T>::mergeSort(Node* const HEAD, const int LENGTH) {
    ...
    Node* pLeft = HEAD;
    Node* pLeftTail = pRight->pPrev;
    pLeftTail->pNext = nullptr;
    pRight->pPrev = nullptr;

    Node* const pLeftSorted = mergeSort(pLeft, LEFT_LEN);
    Node* const pRightSorted = mergeSort(pRight, LENGTH - LEFT_LEN);
    return mergeLists(pLeftSorted, pRightSorted);
}
